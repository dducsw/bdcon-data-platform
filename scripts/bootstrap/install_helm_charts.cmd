...
helm upgrade --install airflow apache-airflow/airflow --namespace data-platform --create-namespace -f ../../helm-values/airflow/dev.yaml --wait --timeout 10m
...
helm repo update
helm upgrade spark-operator kubeflow-spark/spark-operator --namespace data-platform --values ../../k8s/base/platform-services/spark/setup/spark-operator-values.yaml --wait --timeout 180s
echo Apache Spark Kubernetes Operator installed.

echo Installing Trino Cluster (Official Helm Chart)...
helm repo add trino https://trinodb.github.io/charts/
helm repo update
helm upgrade --install trino trino/trino --namespace data-platform --create-namespace -f ../../helm-values/trino/dev.yaml --wait
