...
def main() -> None:
    ...
    for table in TPCDS_TABLES:
        ...
        try:
            ...
            # Run ANALYZE to collect table statistics for CBO and AQE
            print(f"Analyzing statistics for {table}...")
            analyze_sql = f"ANALYZE {env['TRINO_CATALOG']}.{env['TRINO_SCHEMA']}.{table}"
            execute_statement(analyze_sql, env)
            
        except urllib.error.HTTPError as exc:
            raise RuntimeError(f"Failed to create/analyze table {table}: {exc.read().decode('utf-8')}") from exc

    print(
        f"TPC-DS dataset from {env['TPCDS_SOURCE_CATALOG']}.{env['TPCDS_SOURCE_SCHEMA']} "
        f"is ready in Iceberg schema {env['TRINO_CATALOG']}.{env['TRINO_SCHEMA']} (all strings trimmed and statistics analyzed)."
    )
